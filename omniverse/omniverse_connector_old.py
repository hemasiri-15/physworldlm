...
class OmniverseConnector:
    ...
    def initialize(self) -> None:
        ...
        with self._lock:
            ...
            try:
                ...
                # Get the live USD context from the running Omniverse application
                context = self._app_launcher.get_context()

                # Create (or use injected) StageManager
                self._stage_manager = (
                    self._injected_stage_manager
                    or self._build_stage_manager()
                )

                # Bind StageManager to the live USD context
                self._stage_manager.initialize(context)

                # Create an empty stage
                self._stage_manager.create_stage()

                logger.debug("StageManager initialized and empty stage created.")
    ...
    def load_stage(self, path: Union[str, Path]) -> Path:
        ...
        with self._lock:
            ...
            try:
                self._usd_loader.load_usd(str(usd_path))
            except Exception as exc:  # noqa: BLE001
                raise StageLoadError(f"Failed to load USD '{usd_path}': {exc}") from exc

            if self._stage_manager is None:
                raise StageLoadError("load_stage() called but no stage_manager is attached.")

            try:
                self._stage_manager.open_stage(str(usd_path))
            except Exception as exc:  # noqa: BLE001

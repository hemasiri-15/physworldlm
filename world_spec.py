...
PATCH NOTE (ontology grounding integration):
  ...
  to_dict()/from_dict() updated to round-trip it.

PATCH NOTE (sensor integration):
  Added `SensorSpec` and `Entity.sensors: list[SensorSpec]`.
  Backward compatible: `sensors` defaults to `[]`, existing `Entity(...)`
  call sites and existing serialized WorldSpec JSON (no "sensors" key)
  are unaffected. `SensorSpec` deliberately does NOT duplicate any field
  already owned by the sensors/ framework's concrete classes (Camera,
  Lidar, Radar, Imu, Gps, DepthCamera, ThermalCamera) -- resolution,
  fov_deg, noise models, etc. all live in `SensorSpec.params` and are
  validated/defaulted by the concrete Sensor subclass itself when
  SceneCompiler's SensorBuilder constructs it. world_spec.py itself has
  no import dependency on the sensors/ package -- `sensor_type` is a
  plain string, interpreted downstream by SensorBuilder, so a WorldSpec
  can be built/parsed/validated even in an environment without the
  sensors/ package installed.

No existing physics APIs were modified — every field, method signature,
and default on BoundingBox, Vec3, PhysicsState, Environment, Wind,
Interaction, and SimulationGraph is unchanged from the original file.
"""
...
# ─────────────────────────────────────────────
# SensorSpec  (NEW — declares one sensor mounted on an entity)
# ─────────────────────────────────────────────

@dataclass
class SensorSpec:
    """Declares one sensor instance attached to an Entity.

    Minimal by design: only what's needed to CONSTRUCT and MOUNT a
    concrete Sensor subclass via sensors.sensor_manager.SensorManager —
    it does not re-declare any field already owned by that framework's
    classes (resolution, fov_deg, noise models, accuracy, etc. all live
    in `params`, forwarded verbatim as constructor kwargs).

    Attributes:
        sensor_type: Matches sensors.sensor_types.SensorType values —
            "camera", "depth_camera", "thermal_camera", "lidar",
            "radar", "gps", "imu". Kept as a plain string (not the real
            SensorType enum) so world_spec.py has no import dependency
            on the sensors/ package.
        name: Human-readable sensor name (also becomes its USD prim
            name via SceneCompiler's SensorBuilder).
        sensor_id: Optional stable id. Auto-generated by the concrete
            Sensor class if omitted.
        mount_transform: Optional mount offset relative to the owning
            entity, in the same shape as
            sensors.sensor_types.Transform6DoF.to_dict(). Empty dict
            means identity (sensor mounted at the entity's origin).
        params: Forwarded verbatim as **kwargs to the concrete Sensor
            subclass's constructor (e.g. {"resolution": [1920,1080],
            "fov_deg": 90.0} for a Camera). Validated by that class,
            not here.
        enabled: Whether this sensor starts enabled.
    """

    sensor_type: str
    name: str
    sensor_id: Optional[str] = None
    mount_transform: dict = field(default_factory=dict)
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "mount_transform": self.mount_transform,
            "params":          self.params,
    ...
    def from_dict(cls, d: dict) -> "SensorSpec":
        return cls(
            ...
            mount_transform= d.get("mount_transform", {}),
            params=          d.get("params", {}),
...
class Entity:
    ...
    ontology:      dict          = field(default_factory=dict)  # OntologyResolver output
    ...
    sensors:       list[SensorSpec] = field(default_factory=list)  # NEW — sensors mounted on this entity
...
class WorldSpec:
    ...
    def all_sensors(self) -> list[tuple[Entity, SensorSpec]]:
        """Return every (owning_entity, sensor_spec) pair across all
        entities — the flat view SceneCompiler.SensorBuilder /
        SensorManager consume, so callers don't need to nest a loop
        over entities and then over each entity's sensors themselves.
        """
        return [(e, s) for e in self.entities for s in e.sensors]
    ...
    def from_dict(cls, d: dict) -> "WorldSpec":
        ...
        for ed in d.get("entities", []):
            ...
            # Backward compatible: "sensors" key absent in old JSON -> [].
            sensors = [SensorSpec.from_dict(sd) for sd in ed.get("sensors", [])]

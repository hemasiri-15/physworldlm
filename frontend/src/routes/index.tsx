  ...
  return (
    <div className="min-h-screen text-foreground">
      ...
      <main className="relative mx-auto max-w-[1400px] px-4 pb-24 md:px-6">
        ...
        <FutureModules onOpen={setOpenModule} />
        <FutureIntegrations />
      </main>
      <Footer />
      <SuccessToast show={completed} />
      <ModulePanel name={openModule} onClose={() => setOpenModule(null)} />
    </div>
  );
}
      ...
      <div className="mx-auto flex max-w-[1400px] items-center justify-between px-4 py-4 md:px-6">
        <a href="/" className="flex items-center gap-2.5">
          ...
          <div className="flex items-baseline gap-1.5">
            ...
            <span className="rounded border border-white/10 bg-white/5 px-1.5 py-0.5 font-mono text-[10px] text-muted-foreground">
              v0.4
            </span>
          </div>
        </a>
...
function Hero() {
  return (
    <section className="relative pt-16 pb-10 text-center md:pt-20 md:pb-14">
      <div className="mx-auto mb-6 inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/[0.03] px-3 py-1 backdrop-blur-md">
        <Sparkles className="h-3 w-3 text-[color:var(--violet-glow)]" />
      ...
      <h1 className="mx-auto max-w-4xl text-balance text-4xl font-semibold leading-[1.05] tracking-tight md:text-[68px]">
        <span className="text-gradient">Generate Physics-Aware</span>
        <br />
        <span className="text-foreground">Virtual Worlds</span>
      </h1>
      ...
      <div className="mt-8 flex flex-wrap items-center justify-center gap-3">
        ...
        <a
          href="#pipeline"
          className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/[0.03] px-5 py-2.5 text-sm font-medium text-foreground backdrop-blur-md transition-colors hover:bg-white/[0.06]"
        >
...
function Workspace({
  ...
  return (
    <section id="workspace" className="mt-8 grid gap-5 lg:grid-cols-[1.55fr_1fr]">
      ...
      <div className="p-4 md:p-5">
        {tab === "mission" ? (
          ...
          >
            <textarea
              ...
              className="min-h-[200px] w-full resize-none bg-transparent p-5 pr-16 font-sans text-[15px] leading-relaxed text-foreground placeholder:text-muted-foreground/70 focus:outline-none"
            />
            <div className="pointer-events-none absolute right-3 top-3 flex items-center gap-1 rounded-md border border-white/10 bg-white/5 px-2 py-1 font-mono text-[10px] text-muted-foreground opacity-70">
              <ImagePlus className="h-3 w-3" />
              drop images · coming soon
            </div>
          </div>
        ) : (
        ...
        <div className="mt-4 flex flex-col items-stretch gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex items-center gap-2">
            <span className="hidden items-center gap-1 rounded-md border border-white/10 bg-white/5 px-2 py-1 font-mono text-[10px] text-muted-foreground sm:inline-flex">
            ...
            <button
              onClick={onGenerate}
              disabled={generating}
              className="group relative inline-flex items-center gap-2 overflow-hidden rounded-xl px-5 py-3 text-sm font-semibold text-white transition-all hover:scale-[1.02] disabled:opacity-80"
              style={{
            ...
            >
              ...
              {generating ? "Composing world…" : "Generate World"}
...
function AdvancedPanel({ tab }: { tab: TabId }) {
  ...
  return (
    <div className="rounded-xl border border-white/5 bg-black/30 p-2">
      <div className="grid gap-px overflow-hidden rounded-lg bg-white/[0.04] sm:grid-cols-2">
        {rows.map((r) => (
          <div
            key={r.k}
            className="flex items-center justify-between bg-[#0b0b10] px-4 py-3 font-mono text-xs"
          >
...
function IconAction({
  ...
  return (
    <button
      onClick={onClick}
      className="inline-flex items-center gap-1.5 rounded-md border border-white/10 bg-white/[0.03] px-2.5 py-1.5 font-mono text-[11px] text-muted-foreground transition-colors hover:bg-white/[0.06] hover:text-foreground"
    >
...
function highlight(line: string) {
  return line
    ...
    .replace(/(\"[^\"]+\")(\s*:)/g, '<span style="color:oklch(0.78 0.15 215)">$1</span>$2')
    .replace(/:\s*(\"[^\"]*\")/g, ': <span style="color:oklch(0.85 0.12 145)">$1</span>')
    .replace(/:\s*(-?\d+\.?\d*)/g, ': <span style="color:oklch(0.75 0.2 295)">$1</span>')
    .replace(/\b(true|false|null)\b/g, '<span style="color:oklch(0.75 0.2 30)">$1</span>');
}

/* ────────────────────────────── modules ────────────────────────────── */

function FutureModules({ onOpen }: { onOpen: (name: string) => void }) {
  return (
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
        {MODULES.map(({ name, icon: Icon, desc }) => (
          <button
            key={name}
            onClick={() => onOpen(name)}
            className="glass group relative overflow-hidden rounded-xl p-5 text-left transition-all hover:-translate-y-0.5 hover:border-white/15"
          >
            <div className="mb-3 flex h-9 w-9 items-center justify-center rounded-lg border border-white/10 bg-white/5">
              <Icon className="h-4 w-4 text-[color:var(--violet-glow)]" />
            </div>
            ...
            <div className="pointer-events-none absolute inset-x-0 bottom-0 h-px bg-gradient-to-r from-transparent via-[color:var(--violet-glow)]/40 to-transparent opacity-0 transition-opacity group-hover:opacity-100" />
          </button>
        ))}
      </div>
    </section>
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
        {FUTURE_INTEGRATIONS.map((i) => (
          ...
          >
            ...
            <span className="rounded-full border border-white/10 bg-white/5 px-2 py-0.5 font-mono text-[9px] uppercase tracking-widest text-muted-foreground">
              reserved
            </span>
          </div>
        ))}
...
function ModulePanel({ name, onClose }: { name: string | null; onClose: () => void }) {
  const open = !!name;
  return (
    <div
    ...
    >
      ...
      />
      <aside
        className={`glass-strong absolute right-0 top-0 h-full w-full max-w-md overflow-y-auto border-l border-white/10 p-6 transition-transform duration-300 ${
      ...
      >
        <div className="mb-4 flex items-center justify-between">
          ...
          <button
            onClick={onClose}
            className="flex h-8 w-8 items-center justify-center rounded-md border border-white/10 bg-white/5 text-muted-foreground hover:text-foreground"
          >
            <X className="h-4 w-4" />
          </button>
        </div>
        <p className="text-sm leading-relaxed text-muted-foreground">
          {name} is a reserved stage in the PhysWorldLM pipeline. Detailed
          configuration, telemetry, and controls will land here as the runtime
          integrates with OpenUSD and NVIDIA Omniverse.
        </p>
        <div className="mt-5 grid gap-2 font-mono text-xs">
          {["endpoint", "runtime", "schema", "owner"].map((k, i) => (
            <div
              key={k}
            ...
            >
              ...
              <span>
                {["/reserved", "worker · idle", "v0.4.2", "core-team"][i]}
              </span>
            </div>
          ))}
        </div>
        <div className="mt-6 rounded-xl border border-white/10 bg-white/[0.03] p-4 font-mono text-[11px] text-muted-foreground">
          This panel is a placeholder. Wire in module-specific controls when the
          backend for <span className="text-foreground">{name}</span> ships.
        </div>
      </aside>
    </div>
  );
